...
$STD adduser --system --group --home ${APP_HOME} --no-create-home ${APP_USER}
mkdir -p ${APP_HOME}
...
$STD su - postgres -c "psql -c \"CREATE USER ${POSTGRES_USER} WITH PASSWORD '${POSTGRES_PASSWORD}';\""
$STD su - postgres -c "psql -c \"CREATE DATABASE ${POSTGRES_DB} OWNER ${POSTGRES_USER};\""
$STD su - postgres -c "psql -c \"GRANT ALL PRIVILEGES ON DATABASE ${POSTGRES_DB} TO ${POSTGRES_USER};\""
# Grant on public schema for migrations
$STD su - postgres -c "psql -d ${POSTGRES_DB} -c \"GRANT ALL ON SCHEMA public TO ${POSTGRES_USER};\""
...
# Save credentials for service use
...
# Clone or copy from local — we clone from the canonical repo
cd /tmp
$STD git clone --branch Devel --depth 1 https://github.com/lamacheref/OpenIndex.git openindex-src
cp -a /tmp/openindex-src/. ${APP_HOME}/
rm -rf /tmp/openindex-src

msg_info "Creating Python virtual environment"
$STD python3 -m venv ${APP_HOME}/.venv
chown -R ${APP_USER}:${APP_USER} ${APP_HOME}

msg_info "Installing Python dependencies"
$STD ${APP_HOME}/.venv/bin/pip install --upgrade pip setuptools wheel
$STD ${APP_HOME}/.venv/bin/pip install -r ${APP_HOME}/requirements/dev.txt
$STD ${APP_HOME}/.venv/bin/pip install xxhash

msg_info "Building frontend"
$STD npm --prefix ${APP_HOME} install --omit=dev
$STD npm --prefix ${APP_HOME} run build:frontend
...
cd /tmp
$STD wget -q ${POCKETBASE_URL} -O pocketbase.zip
$STD unzip -q pocketbase.zip -d /opt/pocketbase
rm pocketbase.zip
...
msg_info "Creating systemd service: openindex-api"
...
msg_info "Creating systemd service: openindex-indexer-worker"
...
msg_info "Creating systemd service: openindex-indexer-scheduler"
...
msg_info "Creating systemd service: openindex-pocketbase"
...
$STD systemctl enable openindex-pocketbase openindex-api openindex-indexer-worker openindex-indexer-scheduler

# Start in dependency order
$STD systemctl start openindex-pocketbase
sleep 3
$STD systemctl start openindex-api
sleep 2
$STD systemctl start openindex-indexer-worker openindex-indexer-scheduler
